...
# Global variables
_otb_path = "/share/projects/erasmus/deepchange/OTB-7.4.0-Linux64/bin"
...
_gapfilled_dates = "/share/projects/erasmus/deepchange/gapfilled_dates.txt"
_shp_file = "/share/projects/erasmus/deepchange/theiaL2A_zip_img/samples_oso2019_T31TCJ.shp"
_shp_mask_folder = "/share/projects/erasmus/deepchange/theiaL2A_zip_img/TileBorders"
...
if len(sys.argv) == 1:
	...
	print("         ou: ", prog, "  -h")
	print("example command: python processingchain.py -a /media/cacao/SeagateBlack/pepsL2A_zip_img/54HWC -p FALSE")
	print("or")
	print("example command: python processingchain.py -d /media/cacao/SeagateBlack/pepsL2A_zip_img -l 54HWC 54HWD 54HWE -p FALSE")
else:
	usage = " usage: %prog [options] "
	parser = OptionParser(usage=usage)
	parser.add_option("-a", "--atile", dest="atile", action="store", type="string", help="The L2A zip files for a given tile.")
	parser.add_option("-d", "--dir", dest="dir", action="store", type="string", help="The directory of L2A zip files for all tiles.")
	parser.add_option("-o", "--output_dir", dest="output_dir", action="store", type="string", help="The directory where the processed images are stored (the folder that will contain the 'pepsL2A_processed_img' folder).")
	parser.add_option("-l", "--tlist", dest="tlist", action="store", type="string", help="The tiles list to be processed.")
	parser.add_option("-p", "--preview", dest="preview", action="store", type="string", help="Create previews.", default="FALSE")
	parser.add_option("-t", "--perdate", dest="perdate", action="store", type="string", help="Create per date images.", default="FALSE")
...
if options.output_dir == None:
	_l2a_processed_saving_path = os.path.join(os.path.split(os.path.split(tile_folder_path)[0])[0], "theaiL2A_processed_img")
else:
...
if tile_folder_path:
	...
	if (not os.path.exists(stacked_mask)) or preview is True:
		...
		if not os.path.exists(tmp_unzip_path):
			...
			print("====================================================================")
			print("===================== Unzip processing =============================")
			# not unzipping SRE (not corrected for slope) and ATB (athmospheric info from MAJA) files
			cmd = "unzip -u %s/\*.zip -x *_SRE_*.tif *_ATB_*.tif *R2.tif *_IAB_*.tif *_MG2_*.tif -d %s" % (tile_folder_path, tmp_unzip_path)
			try:
				retcode = subprocess.check_call(cmd, shell=True)
			except subprocess.CalledProcessError as e:
				print("======================= Unzip file failed. =======================")
		...
		L2A_gapfilling_main_process(tile, in_dates_list_file, out_dates_list_file, input_path, output_path, otb_path, preview, preview_path, perdate, perdate_path, ram_processing, ram_preview)
	...
	if os.path.exists(stacked_mask) and preview is False:
		...
		# gapfilling function		
		image_timeseries_gapfilling(otb_path, stacked_image, stacked_mask, output_tif, original_dates_file, out_dates_list_file, ram_processing)
	...
	if perdate:
		...
		generate_sqlite_perdate(otb_path, perdate_path, shp_file, shp_mask, field, output_path)
		print("====================================================================")
...
if tiles_dir_path:
	for tile in tile_list:
		...
		if not os.path.exists(tile_folder_path):
			print("========================= Tile error ===================================")
			print("Tile %s is not existing in the given folder %s" % (tile, tiles_dir_path))
			print("========================================================================")
		else:
			tile = os.path.basename(tile_folder_path)
			output_path = os.path.join(_l2a_processed_saving_path, tile)
			...
			if (not os.path.exists(stacked_mask)) or preview is True:
				...
				print("====================================================================")
				print("====================== Gapfilling process ==========================")
				L2A_gapfilling_main_process(tile, in_dates_list_file, out_dates_list_file, input_path, output_path, otb_path,
												preview, preview_path, perdate, perdate_path, ram_processing, ram_preview)
			...
			if os.path.exists(stacked_mask) and preview is False:
				...
				# gapfilling function
				image_timeseries_gapfilling(otb_path, stacked_image, stacked_mask, output_tif, original_dates_file,
											out_dates_list_file, ram_processing)
			...
			if perdate:
				print("====================================================================")
				print("====================== Create SQLite file ==========================")
				generate_sqlite_perdate(otb_path, perdate_path, shp_file, shp_mask, field, output_path, ram_processing)
				print("====================================================================")
				print("The processing of %s has been finished." % tile)
				print("====================================================================")
			else:
