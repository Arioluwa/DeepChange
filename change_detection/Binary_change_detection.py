...
def binary_change_detection(gt_source_path, gt_target_path, outdir_,  case, pred_source_path, pred_target_path):
    ...
    # save the plot
    plt.savefig(os.path.join('./charts','test_change_matrix_percent_case' + case +'.png'))
    ...
    with rasterio.open(os.path.join(outdir_, 'testchange_map_case'+ case +'.tif'), 'w', **profile) as dst:
