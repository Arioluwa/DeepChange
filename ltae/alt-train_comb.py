...
def get_loader(train_dt, val_dt, test_dt, train_dt2, val_dt2, test_dt2, config, ):
    ...
    loader_seq = []
    train_loader = data.DataLoader(data.ConcatDataset([train_dt, train_dt2]), batch_size=config['batch_size'],
                                       num_workers=config['num_workers'],
                                       shuffle=True,
                                          pin_memory=True)
    validation_loader = data.DataLoader(data.ConcatDataset([val_dt, val_dt2]), batch_size=config['batch_size'],
                                       num_workers=config['num_workers'],
                                        shuffle=True,
                                       pin_memory=True)
    test_loader = data.DataLoader(data.ConcatDataset([test_dt, test_dt2]), batch_size=config['batch_size'],
                                       num_workers=config['num_workers'], 
                                        shuffle=True,
                                         pin_memory=True)
...
def main(config):
    ...
    #second sits
    mean_2 = np.loadtxt(glob.glob(config['dataset_folder2'] + '/*mean.txt')[0])
    std_2 = np.loadtxt(glob.glob(config['dataset_folder2'] + '/*std.txt')[0])
    transform2 = transforms.Compose([standardize(mean_2, std_2)])
    
    sits_data2 = glob.glob(os.path.join(config['dataset_folder2'], 'Seed_{}'.format(config['seed'])  + '/*.npz'))
    doy2 = glob.glob(config['dataset_folder2'] + '/gapfilled*.txt')[0]
    
    train_dt2 = SITSData(sits_data2[2], doy2, transform = transform2)
    print("train dataset completed")
    val_dt2 = SITSData(sits_data2[1], doy2,  transform = transform2)
    print("val dataset completed")
    test_dt2 = SITSData(sits_data2[0], doy2,  transform = transform2)
    print("test dataset completed")
    
    
    device = torch.device(config['device'])
    
    loaders = get_loader(train_dt, val_dt, test_dt, train_dt2, val_dt2, test_dt2, config)
...
if __name__ == '__main__':
    ...
    main(config)
    # python alt-train_comb.py --dataset_folder ../../../data/theiaL2A_zip_img/output/2018 --dataset_folder2 ../../../data/theiaL2A_zip_img/output/2019 --res_dir ../../../results/ltae/model/2018_2019 --epochs 15